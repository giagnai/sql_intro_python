...
def fill():
    ...
    conn = sqlite3.connect('secundaria.db')
    #crear cursor para ejecutar las querys
    c = conn.cursor()

    estudiantes = [ ['Pedro', 17, 5, 'Ivan'],
                    ['Juan', 15, 3, 'Kevin'],
                    ['Agustina', 18, 6, 'Manuel'],
                    ['Andrea', 13, 2, 'Ezequiel'],
                    ['Jonas', 15, 3, 'Agustin'] ]    

    c.executemany('''INSERT INTO estudiante (name, age, grade, tutor)
                    VALUES (?,?,?,?);''', estudiantes)
    
    conn.commit() #guardar los cambios realizados en la db
    conn.close()  #cerrar la conexion con la db  
...
def modify(id, name):
    ...
    c.execute('''UPDATE estudiante SET name = '¿Inove?' WHERE id = 2;''')
...
if __name__ == '__main__':
    ...
    new_student = [ ['Martin', 15, 3, 'Antonia'],
                    ['Candela', 18, 6, 'Julio'],
                    ['Franco', 14, 1, 'Marcela'] ]
    # insert(new_student)
    insert(new_student)
